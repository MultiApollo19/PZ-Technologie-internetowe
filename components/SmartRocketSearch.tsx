...
interface SmartRocketSearchProps {
    currentQuery: string;
    currentSort: string;
    currentOrder: string;
}

export default function SmartRocketSearch({ 
    currentQuery, 
    currentSort, 
    currentOrder
}: SmartRocketSearchProps) {
    ...
    const fetchSuggestions = async (searchQuery: string) => {
        ...
        try {
            const response = await fetch(`/api/rocket-suggestions?q=${encodeURIComponent(searchQuery)}`);
    ...
    const updateURL = (searchQuery: string) => {
        ...
        startTransition(() => {
            router.push(`/rockets?${params.toString()}`);
        });
    };
                    ...
                    <form onSubmit={handleSubmit}>
                        <div className="relative">
                            <input
                                ...
                                placeholder="Search rockets (e.g., Saturn V, Atlas, NASA, SpaceX)..."
